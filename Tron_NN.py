...
def scoreCalculation(gameState):
    finalScore = 0
    if (not gameState.tronPlayer1.alive) and gameState.tronPlayer2.alive:
        finalScore += NEURALNET.PLAYER1DEAD
    if gameState.tronPlayer1.alive and (not gameState.tronPlayer2.alive):
        finalScore += NEURALNET.PLAYER2DEAD
    if gameState.tronPlayer1.hitOwnBody:
        finalScore += NEURALNET.PLAYER2SELF
    if gameState.tronPlayer1.alive:
        finalScore += NEURALNET.PLAYER1ALIVE

    return finalScore


# Main function
def playTron(gameState,filePtr):
    ...
    if not gameState.tronPlayer1.alive:
        score2 += 1
        #filePtr('Smart player Wins '+str(score2))
        ...
        score1 += 1
        #filePtr('reflex player Wins '+ str(score1))
    ...
    pygame.time.wait(1)
    #print('Clock Tick value ',gameState.clockTick)
    return scoreCalculation(gameState)+score2+(gameState.clockTick) #+ len(gameState.tronPlayer2.body)
